...
class AutoUpdate extends EventEmitter {
	...
	check() {
		...
		.catch(err => checkDeprecatedPlugin(extensionPath))
		.then(shouldUpdate => {
...
/**
 * Check if given path contains old LiveStyle plugin
 * @param  {String} dir 
 * @return {Promise}
 */
function checkDeprecatedPlugin(dir) {
	return new Promise((resolve, reject) => {
		fs.readdir(dir, (err, items) => {
			// dir exists (no error) and contains old plugin: should update it
			let isOldPlugin = !err && items && items.indexOf('livestyle.py') !== -1;
			debug('contains old plugin? %o', isOldPlugin);
			resolve(isOldPlugin);
		});
	});
}