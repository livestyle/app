...
var detectPlugin = module.exports.plugin = function(app) {
	...
	.then(lists => {
		// Make sure any of the lookup folders exists, which means app is installed.
		// Otherwise throw exception (app does not exists)
		if (!lists.some(item => item)) {
			return Promise.reject(error('ENOSUBLIMETEXT'));
		}

		lists = lists.map(item => item || []);
